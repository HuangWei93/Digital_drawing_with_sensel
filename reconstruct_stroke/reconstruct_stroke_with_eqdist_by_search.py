...
def main():
    ...
    #<velocity-2, force, theta, modified_phi>
    dataset_paras = dataset_paras[:,(2,3,4,5,6)]
    rectangle_size = np.int(sys.argv[1])
    dataset_mean = np.mean(dataset_paras, axis=0)
    dataset_std = np.std(dataset_paras, axis=0)

    def find_texture(para):
        id = np.argmin(np.linalg.norm((dataset_paras - para)/dataset_std, axis = 1))
        texture = dataset_textures[id,:]
        return texture
    ...
    for i, modified_input in enumerate(modified_inputs):
        ...
        for k in range(len(start_ids)):
            ...
            #Reconstruct stroke by searching the nearest patch  with closest input parameters
            for id in range(len(sample_t)):
                x = int(new_x_sample[id]*width/190.0)
                y = int(new_y_sample[id]*height/110.0)
                ...
                src_pts = np.array([[0, 0], [rectangle_size, 0], [rectangle_size, rectangle_size], [0, rectangle_size]], dtype="float32")
